...
def human_error(exc):
    ...
    if "comfortable for some audiences" in low: return "TikTok restricted this post. Only accessible/public media can be downloaded."
    if "sign in" in low or "login required" in low or "authentication" in low: return "This media requires sign-in or authorization."
    if "private" in low: return "This media is private or unavailable to the downloader."
    if "drm" in low: return "This media is DRM-protected and cannot be downloaded."
    if "429" in low or "too many requests" in low or "rate-limit" in low: return "The source temporarily rate-limited this server. Please try again later."
    if "403" in low or "forbidden" in low: return "The source refused automated access to this media."
    if "unsupported url" in low or "no suitable extractor" in low: return "This URL is not supported by the media extractor."
    if "timed out" in low or "timeout" in low: return "The source took too long to respond. Please try again."
    return text[:700] or "Download failed. Please try another media URL."

def ytdlp_options(job, kind, p):
    out=str(WORK/f"{job}.%(ext)s")
    opts={
      "outtmpl":out,"noplaylist":True,"quiet":True,"no_warnings":True,
      "retries":3,"fragment_retries":3,"file_access_retries":2,"socket_timeout":45,
      "concurrent_fragment_downloads":1,"skip_unavailable_fragments":True,
      "restrictfilenames":True,"windowsfilenames":True,"http_headers":{"User-Agent":UA,"Accept-Language":"en-US,en;q=0.9"},
...
def process(job,kind):
    ...
    try:
        p=platform(url); log.info("job=%s platform=%s",job,p)
        with yt_dlp.YoutubeDL(ytdlp_options(job,kind,p)) as ydl: info=ydl.extract_info(url,download=True)
        ...
        db=Session()
        try:
            db.execute(update(Download).where(Download.job_id==job).values(title=title,thumbnail=info.get("thumbnail"),filename=media.name,content_type=ctype,status="completed",error=None))
            db.commit()
        finally: db.close()
...
app=FastAPI(title="QuickDL",version="6.0.0",lifespan=lifespan)
...
def health():
    db=Session()
    try: db.execute(select(Download.id).limit(1)); return {"ok":True,"service":"quickdl","storage":"local-ephemeral","version":"6.0.0"}
    finally: db.close()
...
def create_download(req:DownloadRequest,vexdou_visitor:str|None=Cookie(default=None)):
    ...
    db=Session()
    try: db.add(Download(job_id=job,visitor_id=visitor,url=url,title="Preparing...",status="queued",kind=kind)); db.commit()
    finally: db.close()
...
def history(vexdou_visitor:str|None=Cookie(default=None)):
    ...
    db=Session()
    try:
        # History is intentionally SUCCESS-ONLY. queued/downloading/failed jobs never appear here.
